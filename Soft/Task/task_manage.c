...
void Set_PointFre(u32 Key_Value, u8 *Task_ID)
{
	...
	if (Key_Value == K_2_L || Key_Value == K_2_S)
		Task_First = 1; //更新第一次进入
	if (Task_Index >= Interface)
		Task_Index = 0;
	switch (Task_Index)
	{
	case 0:
...
void Task0_PointFre(u32 Key_Value) //正弦波 (10M) 0-100 000 000
{
	...
	if (Key_Value != K_NO)
	{
		...
		if (SinFre > 1000000000)
			SinFre = 1000000000;				 //数据限制
...
void Task3_SweepFre(u32 Key_Value) //扫频
{
	...
	if (Task_First)
	{
		...
		if (Param_Mode == 0)
			sprintf((char *)fre_buf, "%9d", SweepMinFre);
		if (Param_Mode == 1)
			sprintf((char *)fre_buf, "%9d", SweepMaxFre);
		if (Param_Mode == 2)
			sprintf((char *)fre_buf, "%4d", SweepStepFre);
		if (Param_Mode == 3)
			sprintf((char *)fre_buf, "%3d", SweepTime);
	...
	if (Key_Value != K_NO)
	{
		if (Param_Mode == 0) //最小频率设置
		{
			...
			if (SweepMinFre > SweepMaxFre)
				SweepMinFre = SweepMaxFre;				  //数据限制
		...
		if (Param_Mode == 0)
			...
			Copybuf2dis(showstr, display, P_Index, 0, 0);
		...
		if (Param_Mode == 1) //最大频率设置
		{
			...
			if (SweepMaxFre > 100000000)
				SweepMaxFre = 100000000;				  //数据限制
		...
		if (Param_Mode == 2) //频率步进设置
		{
			...
			if (SweepStepFre > 10000)
				SweepStepFre = 10000 - 1;				   //数据限制
